...
const RawProductOutwardedTable = () => {
  ...
  const [rawOutwardedProducts, setRawOutwardedProducts] = useState([]);
  ...
  return (
    <div className="relative overflow-x-auto mr-[1vw] sm:rounded-lg">
      {loading ? (
        ...
        <DynamicTable headings={headings} rows={rawOutwardedProducts} fromDate={fromDate} toDate={toDate}  />
      )}
    </div>
  );
};

export default RawProductOutwardedTable;
