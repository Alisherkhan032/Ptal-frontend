...
const CreateEngravingOrderPO = () => {
  ...
  const { dropDownProductValue, dropdownProductName, dropDownMatValue, dropDownMatName } = useSelector((state) => state.dropdown);
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const filteredCategories = mode === "product"
          ? response.data.filter((category) => !sfgCategories.includes(category.category_name))
          : response.data.filter((category) => sfgCategories.includes(category.category_name));
    ...
    const fetchItems = async () => {
      ...
      try {
        if (mode === "product") {
          ...
          const response = await rawMaterialServices.getAllRawMaterialsByCatId(selectedCatId);
          dispatch(getAllMaterialByCatIdSuccess(response.data));
        }
      } catch (err) {
        dispatch(mode === "product" ? getAllProductByCatIdFailure() : getAllMaterialByCatIdFailure());
        console.log(err);
      }
    };
    fetchItems();
  ...
  const handleAddOrder = () => {
    if (orderId && quantity && (mode === "product" ? dropDownProductValue : dropDownMatValue) && engravingContent) {
      const newOrder = {
        orderId: orderId,
  ...
  const onFormSubmit = async () => {
    ...
    try {
      const response = await engravingOrderServices.createEngravingOrder(orders);
      if (response?.success) {
        toast.success("Orders created successfully", { autoClose: 1500, onClose: () => window.location.reload() });
  ...
  return (
    <div className="h-[78vh] min-h-[78vh] max-h-[78vh] overflow-y-scroll">
      ...
      <div className="flex flex-row items-center justify-start gap-4 mb-4">
        <button
          className={`px-4 py-2 rounded-lg ${mode === "product" ? "bg-blue-500 text-white" : "bg-gray-300 text-black"}`}
          onClick={() => setMode("product")}
        >
          Products
        </button>
        <button
          className={`px-4 py-2 rounded-lg ${mode === "rawMaterial" ? "bg-blue-500 text-white" : "bg-gray-300 text-black"}`}
      ...
      <div className="flex flex-col p-4 border-2 border-[#F8F6F2] rounded-lg bg-white">
        ...
        <div className="flex flex-col">
          <label>Select {mode === "product" ? "Product" : "Raw Material"}</label>
          {mode === "product" ? (
            <ProductDropdown name="product_id" bgColor={"#F8F6F2"} options={allProductsByCatId} />
          ) : (
            <RawMateialDropDown name="material_id" bgColor={"#F8F6F2"} options={allMaterialsByCatId} />
          )}
        </div>
        <div className="flex flex-col">
        ...
        {errors.listOfOrders && <p className="text-red-500 text-xs">{errors.listOfOrders}</p>}
      </div>
      <div className="mt-4">
              ...
              >
                <div className="flex flex-col">
                  ...
                  <span className="number px-3 text-black">
                    ...
                    <span className="number px-3 text-black">
                      <strong>Engraving Content:</strong> {order?.engravingContent}
                    </span>
...
export default CreateEngravingOrderPO;
